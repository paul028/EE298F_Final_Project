...
from metrics import getMAE, getPRCurve, getMaxFMeasure, getRelaxedFMeasure,own_RelaxedFMeasure
...
def train():
	...
	resume_train = False
	resume_model_path = model_dir + "basnet_bsi_itr_22423_train_3.566629_tar_0.345045.pth"
	last_epoch = 1
	epoch_num = 100000
	batch_size_train = 8
	batch_size_val = 1
	...
	device = torch.device("cuda:1" if torch.cuda.is_available() else "cpu") #set GPU to 1
	# ------- 5. training process --------
	...
	ite_num4val = 1
	next_test = ite_num + 0
	visdom_tab_title = "DefaultBASNetPlots1"
	############
	############
	############
	############
...
#	tra_lbl_name_list = glob.glob(data_dir+ tra_label_dir+ '*'+label_ext)
	...
	#test_lbl_name_list = glob.glob(data_dir+ test_label_dir+ '*'+label_ext)
